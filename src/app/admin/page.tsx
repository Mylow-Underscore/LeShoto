    ...
    <div style={{ minHeight: "100vh", background: C.black, fontFamily: "'Helvetica Neue', Arial, sans-serif", color: C.blanc }}>
      <nav style={{ position: "sticky", top: 0, zIndex: 100, background: "rgba(0,0,0,0.92)", backdropFilter: "blur(12px)", borderBottom: `1px solid ${C.border}`, padding: "0 32px", height: 60, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", gap: 16, alignItems: "center" }}>
          <span style={{ color: C.rose, fontSize: 11, fontWeight: 700, letterSpacing: "0.2em", textTransform: "uppercase" }}>⚙ Admin</span>
          ...
          <motion.button onClick={fetchData} whileTap={{ scale: 0.95 }} style={{ background: C.grisDark, border: `1px solid ${C.border}`, color: C.gris, padding: "6px 14px", fontSize: 11, cursor: "pointer", fontFamily: "inherit", letterSpacing: "0.1em", textTransform: "uppercase" }}>
            ↻ Rafraîchir
          </motion.button>
        </div>
      </nav>
          ...
          {activeTab === "clients" && (
            <motion.div key="clients" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.25 }} style={{ paddingTop: 32 }}>
              ...
              <div style={{ border: `1px solid ${C.border}` }}>
                <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr 1fr 1fr", padding: "10px 18px", background: C.grisMid }}>
                  {["Nom", "Code", "Points", "Bons dispo", "Achats", "Depuis"].map((h) => (
                ...
                {filteredClients.length === 0 ? (
                  ...
                  return (
                    <motion.div key={c.id} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.02 }} style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr 1fr 1fr", padding: "12px 18px", borderTop: `1px solid ${C.border}`, alignItems: "center", cursor: "default" }} onMouseEnter={(e) => { (e.currentTarget as HTMLDivElement).style.background = C.grisDark; }} onMouseLeave={(e) => { (e.currentTarget as HTMLDivElement).style.background = "transparent"; }}>
                      <div>
                        <p style={{ fontWeight: 700, fontSize: 13, margin: "0 0 2px" }}>{c.nomPrenom}</p>
                        {c.email && <p style={{ color: "#444", fontSize: 11, margin: 0 }}>{c.email}</p>}
                      </div>
                      <span style={{ color: C.rose, fontWeight: 700, fontSize: 12, letterSpacing: "0.1em" }}>{c.code}</span>
                      <span style={{ color: pts > 0 ? C.blanc : "#444", fontSize: 13, fontWeight: pts > 0 ? 700 : 400 }}>{pts}</span>
                      <span style={{ color: bons > 0 ? C.warning : "#444", fontSize: 13, fontWeight: bons > 0 ? 700 : 400 }}>
                        {bons > 0 ? `${bons} × ${VALEUR_BON}€` : "—"}
                      </span>
                      <span style={{ color: C.gris, fontSize: 12 }}>{c.achats ?? "0"}</span>
                      <span style={{ color: "#444", fontSize: 11 }}>{formatDate(c.createdAt)}</span>
                    </motion.div>
                  );
                })}
              </div>
